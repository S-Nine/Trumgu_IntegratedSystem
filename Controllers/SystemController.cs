...
namespace Trumgu_IntegratedManageSystem.Controllers
{
    public class SystemController : Controller
    {
        ...
        [HttpPost]
        public JsonResult GetUserToPage(t_sys_userSelObj sel)
        {
            int total = 0;
            List<t_sys_userObj> users = null;
            if (sel.page == null)
            {
                sel.page = 1;
            }
            ...
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            total = db.t_sys_user.Where(rec => rec.is_delete == 0 &&
                   (sel != null && !string.IsNullOrWhiteSpace(sel.name_like) ? rec.name.Contains(sel.name_like) : true))
                .Count();
            if (total > 0)
            {
                users = db.t_sys_user.Where(rec => rec.is_delete == 0 &&
                       (sel != null && !string.IsNullOrWhiteSpace(sel.name_like) ? rec.name.Contains(sel.name_like) : true))
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult AddUser(t_sys_userObj mdl)
        {
            t_sys_userObj user = null;
            ResultObj ro = new ResultObj() { code = (int)EResponseState.TRUMGU_IMS_ERROR_INTERNAL, msg = EResponseState.TRUMGU_IMS_ERROR_INTERNAL.ToString() };
            string cUserInfo = HttpContext.Session.GetString("UserInfo");
            ...
            mdl.last_modify_id = user.id.ToString();
            mdl.last_modify_name = user.name;
            mdl.login_pwd = AESHelper.EncryptText(mdl.login_pwd);

            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            int iCount = db.t_sys_user.Where(rec => rec.is_delete == 0 && rec.login_name == mdl.login_name).Count();
            if (iCount <= 0)
            {
                db.t_sys_user.Add(mdl);
                if (db.SaveChanges() > 0)
                {
                    ro.code = (int)EResponseState.TRUMGU_IMS_SUCCESS;
                    ro.msg = EResponseState.TRUMGU_IMS_SUCCESS.ToString();
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult UpdateUser(t_sys_userObj mdl)
        {
            t_sys_userObj user = null;
            ResultObj ro = new ResultObj() { code = (int)EResponseState.TRUMGU_IMS_ERROR_INTERNAL, msg = EResponseState.TRUMGU_IMS_ERROR_INTERNAL.ToString() };
            string cUserInfo = HttpContext.Session.GetString("UserInfo");
            ...
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            List<t_sys_userObj> list = db.t_sys_user.Where(rec => rec.id == mdl.id && rec.is_delete == 0).ToList();
            if (list != null && list.Count == 1)
            {
                list[0].name = mdl.name;
                list[0].login_name = mdl.login_name;
                if (list[0].login_pwd != mdl.login_pwd)
                {
                    list[0].login_pwd = AESHelper.EncryptText(mdl.login_pwd);
                }
                list[0].tel = mdl.tel;
                list[0].email = mdl.email;
                list[0].gender = mdl.gender;
                list[0].state = mdl.state;
                list[0].last_modify_time = DateTime.Now;
                list[0].last_modify_id = user.id.ToString();
                list[0].last_modify_name = user.name;

                db.t_sys_user.Update(list[0]);
                if (db.SaveChanges() > 0)
                {
                    ro.code = (int)EResponseState.TRUMGU_IMS_SUCCESS;
                    ro.msg = EResponseState.TRUMGU_IMS_SUCCESS.ToString();
        ...
        public JsonResult DeleteUser(int id)
        {
            ...
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            List<t_sys_userObj> list = db.t_sys_user.Where(rec => rec.id == id && rec.is_delete == 0).ToList();
            List<t_sys_relation_delpartment_userObj> list_rel_delp = db.t_sys_relation_delpartment_user.Where(rec => rec.user_id == id && rec.is_delete == 0).ToList();
            List<t_sys_relation_role_userObj> list_rel_role = db.t_sys_relation_role_user.Where(rec => rec.user_id == id && rec.is_delete == 0).ToList();
            ...
            if (list != null && list.Count == 1)
            {
                ...
                // 删除部门和用户关系
                if (list_rel_delp != null && list_rel_delp.Count > 0)
                {
                    for (int i = 0; i < list_rel_delp.Count; i++)
                    {
                        list_rel_delp[i].is_delete = 1;
                        list_rel_delp[i].last_modify_id = user.id.ToString();
                        list_rel_delp[i].last_modify_name = user.name;
                        list_rel_delp[i].last_modify_time = dtNow;
                    }
                    db.t_sys_relation_delpartment_user.UpdateRange(list_rel_delp);
                }

                // 删除角色和用户关系
                if (list_rel_role != null && list_rel_role.Count > 0)
                {
                    for (int i = 0; i < list_rel_role.Count; i++)
                    {
                        list_rel_role[i].is_delete = 1;
                        list_rel_role[i].last_modify_id = user.id.ToString();
                        list_rel_role[i].last_modify_name = user.name;
                        list_rel_role[i].last_modify_time = dtNow;
                    }
                    db.t_sys_relation_role_user.UpdateRange(list_rel_role);
                }
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetUserDelpartment(int id)
        {
            List<TreeDataObj> list = new List<TreeDataObj>();
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            var query1 = db.UserDelpartmentTreeData.FromSql(@"SELECT t.id,t.name,t.parent_id,(SELECT CASE WHEN COUNT(0) > 0 THEN 'true' ELSE 'false' END FROM t_sys_relation_delpartment_user a WHERE a.is_delete = 0 AND t.is_delete =0 AND a.user_id = " + id + " AND a.delpartment_id = t.id) as 'check' FROM t_sys_delpartment t WHERE t.is_delete = 0");
            List<UserDelpartmentTreeDataObj> list1 = query1.ToList();
            if (list1 != null)
            {
                List<UserDelpartmentTreeDataObj> list_root = list1.Where(rec => rec.parent_id == 0).ToList();
                if (list_root != null)
                {
                    for (int i = 0; i < list_root.Count; i++)
                    {
                        TreeDataObj t = new TreeDataObj();
                        t.id = list_root[i].id.ToString();
                        t.text = list_root[i].name;
                        t.state = "";
                        t.check = bool.Parse(list_root[i].check);
                        t.attributes = list_root[i].parent_id.ToString();
        ...
        /// 递归组织机构树
        /// </summary>
        private List<TreeDataObj> RecursionDelpartment(List<UserDelpartmentTreeDataObj> list, int? id)
        {
            List<TreeDataObj> childer = new List<TreeDataObj>();
            if (list != null && list.Count > 0)
            {
                List<UserDelpartmentTreeDataObj> temp = list.Where(rec => rec.parent_id == id).ToList();
                if (temp != null)
                {
                    for (int i = 0; i < temp.Count; i++)
                    {
                        TreeDataObj t = new TreeDataObj();
                        t.id = temp[i].id.ToString();
                        t.text = temp[i].name;
                        t.state = "";
                        t.check = bool.Parse(temp[i].check);
                        t.attributes = temp[i].parent_id.ToString();
                        t.children = RecursionDelpartment(list, temp[i].id);
                        childer.Add(t);
                    }
                }
            }
        ...
        /// </summary>
        [HttpPost]
        public JsonResult DistributionUserDelp(int user_id, List<int> list)
        {
            t_sys_userObj user = null;
            DateTime dtNow = DateTime.Now;
            ResultObj ro = new ResultObj() { code = (int)EResponseState.TRUMGU_IMS_ERROR_INTERNAL, msg = EResponseState.TRUMGU_IMS_ERROR_INTERNAL.ToString() };
            ...
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            List<t_sys_userObj> mdl = db.t_sys_user.Where(rec => rec.is_delete == 0 && rec.id == user_id).ToList();
            if (mdl != null && mdl.Count > 0)
            {
                List<t_sys_relation_delpartment_userObj> list_add_rel_delp = new List<t_sys_relation_delpartment_userObj>();
                // 删除原有的部门关系
                List<t_sys_relation_delpartment_userObj> list_rel_delp = db.t_sys_relation_delpartment_user.Where(rec => rec.is_delete == 0 && rec.user_id == user_id).ToList();
                // 删除部门和用户关系
                if (list_rel_delp != null && list_rel_delp.Count > 0)
                {
                    for (int i = 0; i < list_rel_delp.Count; i++)
                ...
                if (list != null && list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        t_sys_relation_delpartment_userObj t = new t_sys_relation_delpartment_userObj();
                        t.delpartment_id = list[i];
                        t.user_id = user_id;
                        t.is_delete = 0;
                        t.last_modify_id = user.id.ToString();
                        t.last_modify_name = user.name;
                        t.last_modify_time = dtNow;
                        list_add_rel_delp.Add(t);
                    }
                    db.t_sys_relation_delpartment_user.AddRange(list_add_rel_delp);
                }
                if (db.SaveChanges() > 0)
                {
                    ro.code = (int)EResponseState.TRUMGU_IMS_SUCCESS;
        ...
        /// </summary>
        [HttpPost]
        public JsonResult GetUserRole(int id)
        {
            List<TreeDataObj> list = new List<TreeDataObj>();
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
            var query1 = db.UserRoleTreeData.FromSql(@"SELECT t.id,t.name,(SELECT CASE WHEN COUNT(0) > 0 THEN 'true' ELSE 'false' END FROM t_sys_relation_role_user a WHERE a.is_delete = 0 AND a.user_id = " + id + " AND t.is_delete = 0 AND a.role_id = t.id) AS 'check' FROM t_sys_role t WHERE t.is_delete = 0;");
            List<UserRoleTreeDataObj> list1 = query1.ToList();
            if (list1 != null)
            {
                for (int i = 0; i < list1.Count; i++)
                {
                    TreeDataObj t = new TreeDataObj();
                    t.id = list1[i].id.ToString();
                    t.text = list1[i].name;
                    t.state = "";
                    t.check = bool.Parse(list1[i].check);
                    list.Add(t);
                }
            }
            return Json(list);
        ...
        /// </summary>
        [HttpPost]
        public JsonResult DistributionUserRole(int user_id, List<int> list)
        {
            t_sys_userObj user = null;
            DateTime dtNow = DateTime.Now;
            ResultObj ro = new ResultObj() { code = (int)EResponseState.TRUMGU_IMS_ERROR_INTERNAL, msg = EResponseState.TRUMGU_IMS_ERROR_INTERNAL.ToString() };
            ...
            if (mdl != null && mdl.Count > 0)
            {
                List<t_sys_relation_role_userObj> list_add_rel_role = new List<t_sys_relation_role_userObj>();
                // 删除原有的角色关系
                List<t_sys_relation_role_userObj> list_rel_role = db.t_sys_relation_role_user.Where(rec => rec.is_delete == 0 && rec.user_id == user_id).ToList();
                // 删除部门和用户关系
                if (list_rel_role != null && list_rel_role.Count > 0)
                {
                    for (int i = 0; i < list_rel_role.Count; i++)
                ...
                if (list != null && list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        t_sys_relation_role_userObj t = new t_sys_relation_role_userObj();
                        t.role_id = list[i];
                        t.user_id = user_id;
                        t.is_delete = 0;
                        t.last_modify_id = user.id.ToString();
                        t.last_modify_name = user.name;
                        t.last_modify_time = dtNow;
                        list_add_rel_role.Add(t);
                    }
                    db.t_sys_relation_role_user.AddRange(list_add_rel_role);
                }
                if (db.SaveChanges() > 0)
                {
                    ro.code = (int)EResponseState.TRUMGU_IMS_SUCCESS;
        ...
        /// </summary>
        [HttpPost]
        public JsonResult UpdatePWD(string o_pwd, string n_pwd)
        {
            t_sys_userObj user = null;
            DateTime dtNow = DateTime.Now;
            ResultObj ro = new ResultObj() { code = (int)EResponseState.TRUMGU_IMS_ERROR_INTERNAL, msg = EResponseState.TRUMGU_IMS_ERROR_INTERNAL.ToString() };
            ...
            if (!string.IsNullOrWhiteSpace(o_pwd) && !string.IsNullOrWhiteSpace(n_pwd))
            {
                o_pwd = AESHelper.EncryptText(o_pwd);
                n_pwd = AESHelper.EncryptText(n_pwd);
                Utils.DataContextHelper db = Utils.DBHelper.CreateContext();
                List<t_sys_userObj> list = db.t_sys_user.Where(rec => rec.id == user.id && rec.is_delete == 0 && rec.login_pwd == o_pwd).ToList();
                if (list != null && list.Count == 1)
                {
                    list[0].login_pwd = n_pwd;
                    db.t_sys_user.Update(list[0]);
                    if (db.SaveChanges() > 0)
                    {
                        ro.code = (int)EResponseState.TRUMGU_IMS_SUCCESS;
