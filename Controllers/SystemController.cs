...
namespace Trumgu_IntegratedManageSystem.Controllers {
    public class SystemController : Controller {
        ...
        public JsonResult GetUserToPage (t_sys_userSelObj sel) {
            var R=Request;
            List<t_sys_userObj> user_ary = null;
            Utils.DataContextHelper db = Utils.DBHelper.CreateContext ();
            int total = db.t_sys_user.Where (rec => rec.is_delete == 0 &&
                (string.IsNullOrWhiteSpace (sel.name_like) ? true : rec.name.Contains (sel.name_like))
            ).Count ();
            if (total > 0) {
                user_ary = db.t_sys_user.Where (
                    rec => rec.is_delete == 0 &&
                    (string.IsNullOrWhiteSpace (sel.name_like) ? true : rec.name.Contains (sel.name_like))
                ).OrderByDescending (rec => rec.id).Skip ((int) ((sel.page - 1) * sel.rows)).Take ((int) sel.rows).ToList ();
            } else {
                user_ary = new List<t_sys_userObj> ();
            }

            db.Dispose ();
            return Json (new { total = total, rows = user_ary });
        }
    }
}