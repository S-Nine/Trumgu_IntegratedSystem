...
function initDatagrid() {
    $('#role_datagrid').datagrid({
        url: '/PFXFundOrganization/GetRoleToList/',
...
function initEvent() {
        ...
        toolbar: [{
            ...
            handler: function() {
                if (isValidate('dlg_add_role')) {
                    ...
                    var params = {
                        id: $('#hid_id').val(),
                        role: $('#txt_name').textbox('getValue'),
                        data_authority: $('#cmb_data_permiss').combobox('getValue'),
                        status: $('#cmb_state').combobox('getValue'),
                        rolecode: $('#txt_rolecode').textbox('getValue')
                    };
                    $.ajax({
                        url: params.id == null || params.id == '' ? '/PFXFundOrganization/AddRole/' : '/PFXFundOrganization/UpdateRole/',
                        type: 'post',
                        dataType: 'json',
                        data: params,
        ...
        toolbar: [{
            ...
            handler: function() {
                if (isValidate('dlg_add_right')) {
                    ...
                    $.ajax({
                        url: '/PFXFundOrganization/DistributionRight/',
                        type: 'post',
                        dataType: 'json',
    ...
    $('#tree_right').tree({
        checkbox: true,
        lines: true,
        url: '/PFXFundOrganization/GetRoleMenuRight/',
        method: 'POST',
        animate: true,
...
function initButtonEvent() {
    ...
    $('#btn_edit') != null && $('#btn_edit').click(function() {
        ...
        if (rows != null && rows.length == 1) {
            ...
            $('#txt_name').textbox('setValue', rows[0].role != null ? rows[0].role : '');
            $('#cmb_data_permiss').combobox('setValue', rows[0].data_authority != null ? rows[0].data_authority : 0);
            $('#cmb_state').combobox('setValue', rows[0].status != null ? rows[0].status : 1);
            $('#txt_rolecode').textbox('setValue', rows[0].rolecode != null ? rows[0].rolecode : '');
    ...
    $('#btn_del') != null && $('#btn_del').click(function() {
        ...
        if (rows != null && rows.length == 1) {
            $.messager.confirm('确认', '您确认想要删除角色数据吗？', function(r) {
                if (r) {
                    $.ajax({
                        url: '/PFXFundOrganization/DeleteRole/',
                        type: 'post',
                        dataType: 'json',
                        data: { id: rows[0].id },
                        success: function(data) {
