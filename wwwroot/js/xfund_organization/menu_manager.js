...
function initMenuTreegrid() {
    $('#menu_treegrid').treegrid({
        url: '/XFundOrganization/GetMenuToList/',
        idField: 'id',
        treeField: 'menu',
        ...
        onClickRow: function(row) {
            ...
            if (row.id != null && $.trim(row.id) != '' && !isNaN(row.id)) {
                $('#button_datagrid').datagrid({ url: '/XFundOrganization/GetMenuButton/', queryParams: { menu_id: row.id } });
            }
        }
    });
}
        ...
        toolbar: [{
            ...
            handler: function() {
                if (isValidate('dlg_add_menu')) {
                    ...
                    $.ajax({
                        url: params.id == null || params.id == '' ? '/XFundOrganization/AddMenu/' : '/XFundOrganization/UpdateMenu/',
                        type: 'post',
                        dataType: 'json',
                        data: params,
        ...
        toolbar: [{
            ...
            handler: function() {
                if (isValidate('dlg_add_button')) {
                    ...
                    $.ajax({
                        url: params.id == null || params.id == '' ? '/XFundOrganization/AddButton/' : '/XFundOrganization/UpdateButton/',
                        type: 'post',
                        dataType: 'json',
                        data: params,
                        success: function(data) {
                            ...
                            if (data != null && data.code == 200) {
                                ...
                                $.messager.alert('提示', '保存成功！');
                                $('#button_datagrid').datagrid({ url: '/XFundOrganization/GetMenuButton/', queryParams: { menu_id: params.menu_id } });
                            } else {
                                $.messager.alert('提示', '保存失败！');
                            }
                        },
    ...
    $('#btn_del_menu') != null && $('#btn_del_menu').click(function() {
        ...
        if (rows != null && rows.length == 1) {
            $.messager.confirm('确认', '您确认想要删除菜单数据吗？', function(r) {
                if (r) {
                    $.ajax({
                        url: '/XFundOrganization/DeleteMenu/',
                        type: 'post',
                        dataType: 'json',
                        data: { id: rows[0].id },
                        success: function(data) {
    ...
    $('#btn_del_button') != null && $('#btn_del_button').click(function() {
        ...
        if (rows != null && rows.length == 1) {
            $.messager.confirm('确认', '您确认想要删除按钮数据吗？', function(r) {
                if (r) {
                    $.ajax({
                        url: '/XFundOrganization/DeleteButton/',
                        type: 'post',
                        dataType: 'json',
                        data: { id: rows[0].id },
                        success: function(data) {
                            if (data != null && data.code == 200) {
                                $.messager.alert('提示', '删除成功！');
                                $('#button_datagrid').datagrid({ url: '/XFundOrganization/GetMenuButton/', queryParams: { menu_id: rows_menu[0].id } });
                            } else {
                                $.messager.alert('提示', '删除失败！');
                            }
                        },
