...
function initDatagrid() {
    $('#role_datagrid').datagrid({
        url: '/XFundOrganization/GetRoleToList/',
...
function initEvent() {
        ...
        toolbar: [{
            ...
            handler: function () {
                if (isValidate('dlg_add_role')) {
                    ...
                    $.ajax({
                        url: params.id == null || params.id == '' ? '/XFundOrganization/AddRole/' : '/XFundOrganization/UpdateRole/',
                        type: 'post',
                        dataType: 'json',
                        data: params,
        ...
        toolbar: [{
            ...
            handler: function () {
                if (isValidate('dlg_add_right')) {
                    ...
                    $.ajax({
                        url: '/XFundOrganization/DistributionRight/',
                        type: 'post',
                        dataType: 'json',
    ...
    $('#tree_right').tree({
        checkbox: true,
        lines: true,
        url: '/XFundOrganization/GetRoleMenuRight/',
        method: 'POST',
        animate: true,
    ...
    $('#btn_del') != null && $('#btn_del').click(function () {
        ...
        if (rows != null && rows.length == 1) {
            $.messager.confirm('确认', '您确认想要删除角色数据吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: '/XFundOrganization/DeleteRole/',
                        type: 'post',
                        dataType: 'json',
                        data: { id: rows[0].id },
                        success: function (data) {
